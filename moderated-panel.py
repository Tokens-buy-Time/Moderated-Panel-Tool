...
# prompt += "\n\n(a03) The Moderator will use the user-provided topic to develop a series of 20 questions."
# prompt += " \n\n(a04) The questions will be presented to the panel members after the user switches to voice mode."
# prompt += "\n(a04) Each question will then be put to an individual panel member, one member at a time, while in voice mode."
# prompt += "\n\n(a03) The moderator is to put their 20 questions to the panel such that for each question the moderator will choose a particular panel member to kick off the conversation as the moderator determine most appropriate."
...
[
    ...
    (a01) have the panel discuss, using Conversational AI, where the participants are as previously defined,

    (a02) discus any given topic, where the topic is to be developed - from a brief, supplied by the Device User, as the initial input and is to be used to supplement the complete engineered prompt - which defines the overall assignment - as raw training data contained within the uploaded document and committed to memory.

    (a03) the Moderator using the user provided briefing, PDF file uploaded, to develop the aforementioned series of 20 questions, to be presented to the panel members after the user switches to voice mode,
    (a04) each question then to be put to an individual panel member; one member at a time, while in voice mode.
